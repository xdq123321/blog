<?php
	...
	?>
	<?php
		@$val=isset($_GET['search_inp'])?trim($_GET['search_inp']):'';
		?>
<!DOCTYPE html>
<html>
	<head>
		...
		<title>关于  "<?php echo $val?>" 的文章 | 肥小波博客</title>
		...
		<div class="container">
			<div class="row">
				<div class="col-lg-9">
							...
							<h2 class="new_article">共 <?php 
							$sql="select * from article where article_content like '%{$val}%' or article_type like '%{$val}%'";
								$result=mysqli_query($link,$sql);
								 $rows = $result->num_rows;
							echo $rows?> 条关于" <?php echo $val?> "的文章</h2>
								<?php
								Page($rows,8);
								$sql = "select * from article a,article_details b where a.article_id=b.details_id and (article_content like '%{$val}%' or article_type like '%{$val}%') order by article_time desc limit $select_from $select_limit";
					$result=mysqli_query($link,$sql);
					//5
    ...
    echo '<li><a href="search.php?page='.$pre_page.'&&search_inp='.$val.'" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';}
	for ($i=1; $i <=$page_count; $i++) {
		if($i==$page){
			echo '<li class="active"><span><span aria-hidden="true">'.$i.'</span></span></li>';
		}else{
			echo '<li><a href="search.php?page='.$i.'&&search_inp='.$val.'">'.$i.'</a></li>';
		} 
    ...
    echo '<li><a href="search.php?page='.$next_page.'&&search_inp='.$val.'" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
	}
									    	?>
    ...
    <script type="text/javascript">
    	var keyw=<?php echo json_encode($val)?>;
    	var inp=document.getElementById("jump_pagee");
    	inp.onchange=function(){
    		var inp_val=inp.value;
    		window.location.href="search.php?page="+inp_val+"&&search_inp="+keyw;	
    	}
    </script>
	</body>
</html>
